...
| 6 | Reranking | 10m | Two-stage: top 50-100 (retrieve) → top 5-10 (rerank) → LLM. Cross-encoder sees query + doc together = richer signal than bi-encoder (which embeds separately). Native in ES (Caseway anchor); Cohere/BGE as managed options. Trade-off: latency cost for precision lift. | cross-encoder, bi-encoder, two-stage retrieval, Cohere Rerank, BGE-Reranker | "After hybrid retrieval, what's the next-highest-ROI lever and what does it do mechanically that pure retrieval can't?" → Cross-encoder reranker; scores query+doc together vs separate embeddings = richer relevance signal |
| 7 | Context assembly + citations | 10m | Dedup near-duplicates (near-duplicate threshold). Order by relevance — most-relevant at start AND end (combat lost-in-the-middle). Citations mandatory for regulated domains. Handle "no relevant context" with explicit "I don't know" — don't hallucinate. | citation, deduplication, lost-in-the-middle, "I don't know" fallback | "If retrieval returns no relevant chunks, what should production RAG do?" → Return "I don't have enough information to answer" — never let LLM hallucinate |
| 8 | RAG evaluation | 8m | **RAGAS** = production-standard eval framework. **Killer feature: LLM-as-judge → no ground-truth labels needed.** Measures retrieval quality + answer quality (specific metric names = recognition, not recitation). Recall@k/MRR as standard retrieval-side metrics. | RAGAS, LLM-as-judge, faithfulness, answer relevancy, context precision, context recall, recall@k, MRR | "What does RAGAS measure WITHOUT ground truth, and why is that the breakthrough?" → LLM-as-judge for the answer-quality + retrieval-quality metrics; eval on production traffic without human labeling cost |
| 9 | Advanced patterns | 13m | **HyDE** (Hypothetical Document Embeddings) — embed a hypothetical answer not the question; answer-text matches answer-text better than question-text matches answer-text. **Query rewriting** (spelling, expansion, multi-query) — single bullet. **Parent-document retrieval** — index small chunks, return parent for generation context. | HyDE, query rewriting, multi-query, parent-document retrieval | "Explain HyDE in one sentence." → Generate a hypothetical answer, embed THAT instead of the question; answer-text matches answer-text better than question-text matches answer-text |
...
| # | Chunk | Tier | ~Time | Teach | Key vocab | Check question |
|---|---|---|---|---|---|---|
| 1 | What "governance" means for an agentic system | T1 | 15m | Governance = the policies, controls, and traceability that make autonomous systems SAFE to deploy. Not about restricting agents — about making them auditable + correctable + accountable. Three pillars: WHAT they can do, WHO approves, HOW you trace. | governance, autonomy, accountability, traceability | "Define governance for an agentic system in one sentence — your words." → Policies + controls + traceability that make autonomous systems safe to deploy = auditable, correctable, accountable |
| 2 | Role boundaries + HITL | T1 | 15m | Each agent has explicit role + permissions. Enrichment agent can't take destructive action. Response agent can't modify detection rules. HITL where: irreversible actions, low confidence, high-risk operations. Identity-aware (RBAC). | role boundary, least privilege, HITL, identity-aware orchestration, approval gate | "Why are role boundaries part of governance, not just clean architecture?" → Boundaries prevent capability creep and limit blast radius of compromise/hallucination; auditable separation of concerns |
| 3 | Audit trails + accountability | T1 | 15m | Every agent decision logged: timestamp, agent ID + version, inputs, tool calls + args, outputs, confidence, HITL approvals (if any), policy violations triggered. Async log sink — never blocks. Used for: incident investigation, compliance, drift detection, training data. | audit trail, observability, compliance, decision logging, post-incident investigation | "An agent hallucinated a tool call. What 5 fields in your audit trail help investigate?" → timestamp, agent ID + version, tool name + args, tool result, confidence score, HITL gate state, prompt hash |
| 4 | Escalation policies + tiered autonomy | T1 | 15m | Map (confidence, risk) → tier → who approves. Time-boxed decision windows (30 min low-risk, 5 min emergency). Define what gets auto-escalated when (timeout, low confidence, novel scenario). | tiered autonomy, escalation policy, time-boxed approval, RBAC | "How do you decide what gets auto-action vs HITL?" → Map (confidence × risk) → tier; high conf + low risk = auto; mid = HITL approval; low conf OR destructive = full review |
