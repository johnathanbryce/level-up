...
**Current Sub-topic:** Algorithms: Python & JS Phase 1 in progress. System Design: Architectural Patterns COMPLETE. Next System Design sub-topic: **Caching** (cache-aside, write-through, eviction, Redis, CDN, stampede).
**Last Session Summary:** **Algorithms:** Python Word Frequency Counter — correct two-pass solution (count then filter). Still defaulted to `if/else` for counting but successfully refactored to `.get(word, 0) + 1` after prompt. Wrote first dict comprehension correctly with no syntax hints — good milestone. Light Monday warmup (one problem). **Applied recall:** Trello clone MVP scenario — correctly argued against microservices for 3-person team, identified speed/operational overhead. Confused monorepo with monolith — corrected. **System Design:** Architectural Patterns completed — all 5 chunks (monolith, microservices, decision framework, serverless, event-driven). Applied scenario: healthtech patient portal. Strengths: correct monolith choice, correct sync/async split for most features, knew file uploads should be event-driven. Gaps: tangent on on-prem/vector DB (interview rabbit hole), missed audit logging as synchronous compliance requirement, "50,000K" units error, attributed serverless rejection to cold starts instead of cost. Learned: monorepo ≠ monolith, HIPAA-compliant cloud is standard, audit logs can't be async.
**Next Session Plan:** Regular algo warmup (rotation: Python is up — continue Phase 1, reinforce `.get()`, more dict/list comprehension practice). Applied recall from Architectural Patterns (monolith vs micro decision, serverless cost model, or sync vs async split). System Design next sub-topic: **Caching**. Chunked teaching pace applies.
**Notes:** John takes notes in per-topic .md files under each section's notes/ folder. Python writing fluency is the main skill to rebuild — comprehension is ahead of production. Prefers lean/practical coverage of system design topics; pushes back when something feels theoretical and asks for justification — this is healthy. Honor that pattern: research, give honest answers, adjust scope rather than insisting. **Teaching pace (saved as feedback memory 2026-04-08):** never dump all stages of a sub-topic at once; teach one chunk at a time with confirmation between chunks. **Light Monday warmups (saved as feedback memory 2026-04-13):** one algo problem on Mondays, save harder sessions for mid-week. **Recurring weak spot — arithmetic/units execution:** confirmed across multiple sessions; now includes units errors ("50,000K"). Reinforce on every numeric question: write each step, give the actual number, sanity-check magnitude and units. **"Knows the rule, hasn't internalized implications":** when John recites a principle, push past the rule statement and force prediction of concrete failure modes — that's the mid→senior gap. Strong architectural instincts deserve explicit naming when they appear so the vocabulary sticks. **Interview tangent tendency:** in applied scenarios, John can go on tangents (on-prem, vector DBs) that are interesting but off-topic — gently redirect. Two-pointer (opposite-direction) is now unblocked in JS; same-direction still bookmarked for Phase 2. **New JS fundamental gap (2026-04-09):** expression vs assignment — `x + 1` vs `x = x + 1` / `x += 1`. Now corrected, watch for recurrence. **New vocabulary correction (2026-04-13):** monorepo ≠ monolith. Watch for conflation.
