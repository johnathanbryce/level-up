...
**Current Section:** Section 1 (Algorithms — ongoing) + Section 2 (System Design — **REVIEW PHASE in progress: 3 of 12 sections quizzed**)
**Current Sub-topic:** Core Concepts (Vertical/Horizontal Scaling, CAP Theorem, Consistency Models, Latency vs Throughput). John must solo-study first. Next: quiz on Core Concepts.
**Last Session Summary:** **Python algo (temperature_spikes.py — difference-flavor hash-map-complement, B+, NOT clean rep #3) + Section 3 review (Back-of-Envelope Estimation, 4 of 6 Qs, C/C+) + 5 new sub-sections added to `03-back-of-envelope-estimation.md` mid-quiz + Section 3 questions appended to `interview-questions.md`.**
...
**Next Session Plan:** Core Concepts review. John solo-studies first. Open items to weave in:
- **CAP theorem + chat apps** — Q4 of original session, John initially chose CP for chat (corrected to AP). Re-test cold.
- **Rule→implications drill on every applied answer** — multi-step chain is the weak muscle. Force "what specifically does that mean for the DB / LB / app server?"
- **Consistent hashing cache-locality fix** → Load Balancing review still outstanding (2nd occurrence stubborn gap)
...
- **Hash-map-complement variant rep #4 (Python)** → next Python rotation. Lock targets: single complement direction (no ±k hedge), no nested-loop question.
