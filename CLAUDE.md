...
**Current Section:** Section 1 (Algorithms — ongoing) + Section 2 (System Design — **REVIEW PHASE in progress: 8 of 12 sections quizzed**)
**Current Sub-topic:** Database Architecture — next to review. John solo-studies first.
**Last Session Summary (2026-05-13):** JS algo warmup — **frequent_workers.ts (Frequency Counting count-then-inspect drill in JS). B+.** `(map.get(k) || 0) + 1` idiom used correctly first try — no verbose if/else (this recurring weak spot now locked in JS). Filter+map chain with destructuring clean. Two fixes needed: missing `.sort()` (missed the constraint despite being pointed to it) and redundant empty-check ternary. Bridge question: consistent hashing vs session stickiness — C (4th occurrence of conflation; framed it as a session/user-state problem instead of key-locality problem; didn't name consistent hashing). **Message Queues & Async Processing review: B+ overall (2 questions, section scoped lean per John's correct calibration call — supporting vocabulary, not standalone interview topic).** Q2 applied scenario B+: all four sync/async calls correct; gap: step 3 requires two profile updates (raw URL sync, thumbnail URL async after step 2). Q3 at-least-once B+/A-: mechanism + idempotency named correctly; minor gap: didn't state the pre-processing check explicitly. Notes update: added `### Common Confusion: Stickiness vs. Consistent Hashing` table + one-liner + interview diagnostic to `07-load-balancing-and-networking.md`.
...
**Next Session Plan:** Resilience & Reliability review (circuit breaker three-state machine MUST be tested — outstanding since 2026-04-22). Then Observability review (lean, 2-3 questions). After both done: commit + begin interview-questions.md backfill for DB Architecture and Search Infrastructure sections.

**Open items still outstanding (carry forward to relevant section reviews):**
- **Consistent hashing / session stickiness conflation — 3rd occurrence** → Confirmed stubborn gap. Retest in master pre-case-study quiz with a fresh scenario.
- **Percentile gotcha** → Observability review (5th rule→implications)
- **Circuit breaker three-state machine** → Resilience review — OUTSTANDING SINCE 2026-04-22. Must quiz today.
...
- **Cache stampede single-hot-key fix (lock-based recomputation)** → Capstone Prep. 2nd confirmed occurrence. Tier 1. Must know: single hot key → distributed lock (SET NX). Jitter is wrong tool for a single key.
- **CDN cache busting / content-addressed filenames** → Capstone Prep. Named purge API as correct fix — it's the naive fix. Correct: filename includes content hash, long TTL, HTML has short TTL.
- **BM25 vs embeddings conflation — 2nd occurrence (2026-05-14)** → Tier 1, critical before Section 5. Reached for ES for semantic search in DB Architecture Q6 AND Search Infrastructure Q1. Must be clean: BM25 = keyword relevance (ES), semantic = vector embeddings (pgvector/vector DB).
- **Replica lag = normal async behavior** → Described crash/failover instead. Re-test at master quiz.
- **conversation_id > message_id for messaging app sharding** → Missed in DB Architecture Q4. Re-test at master quiz.
- **Filter-vs-search diagnostic** → Before recommending ES, ask: "Is this relevance-ranked freeform search, or structured filtered lookups?" Most B2B record queries are WHERE clauses, not search problems.
- **interview-questions.md backfill** → DB Architecture and Search Infrastructure questions not yet added. Do after today's session.
