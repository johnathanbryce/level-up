...
**Current Sub-topic:** Algorithms: Python & JS Phase 1 in progress. System Design: Request Lifecycle COMPLETE. Next System Design sub-topic: Core Concepts (vertical/horizontal scaling, CAP theorem, consistency models, latency vs throughput).
**Last Session Summary:** Long session split across morning + evening. **Algorithms (morning):** JS Valid Palindrome — solved reverse-and-compare cleanly, then re-taught two-pointer (opposite-direction) from first principles after John admitted he hadn't really understood it from the prior day. Major breakthrough: he wrote the two-pointer version cleanly on first attempt after the lesson; pattern is now unblocked in JS (still needs Python demonstration to count as fully complete in both). Python Valid Anagram — built all 3 approaches (sorted, manual dict, Counter); surfaced the Python `==` deep-equality vs JS reference-equality gotcha unprompted. Notes written: `01-algorithms/notes/two-pointer.md` and `01-algorithms/notes/python/dict-patterns.md`. **System Design (evening):** Full Request Lifecycle walked end-to-end across all 6 stages — DNS, TCP, TLS, HTTP request, server processing, browser render. Strong session, A-/B+ across the board. Notable moments: senior-flavor question on whether DNS cache hits reset TTL (they don't — absolute time-from-fetch); blanked on TCP recall and conflated it with TLS encryption (re-taught cleanly); appropriate pushback on SYN/ACK depth which is correctly senior+; C+ on the GET-as-DELETE failure modes question (knew the rule but didn't predict consequences — important "knows the rule, hasn't internalized implications" gap); A- walking the LB→app→cache→DB diagram for latency bottlenecks; B+ async/defer fix. HTTP/3 versions deferred as out-of-scope. Latency math undercounted again (parallel to yesterday's QPS miss) — pattern: reasoning ahead of arithmetic execution. **Notes hygiene:** Converted all 3 system-design notes from bold-as-section to proper `##` header hierarchy after John flagged the formatting inconsistency. Standard locked: `#` for file title, `##` for major sections, `###` for sub-sections, **bold** reserved for vocabulary/emphasis only. Code blocks always fenced with language tag.
**Next Session Plan:** **Bonus JS hash map challenge FIRST** (John explicitly requested another rep on the hash map pattern before the regular warmup — he wants to lock in the "replace search with lookup" mental model that he just got walked through on Two Sum). Suggested options: Valid Anagram in JS (uses Map for char counts — also bridges to the Python version he already did), First Unique Character (Map for counts then second pass), or Group Anagrams (Map of sorted-key → list — slightly harder, may be too much for one bonus rep). After the bonus, continue with the regular daily algo warmup. Then **applied recall question** pulled from Request Lifecycle. Then System Design Core Concepts: vertical vs horizontal scaling, CAP theorem in practice, consistency models (strong vs eventual), latency vs throughput. Apply chunked teaching pace — feedback memory was created on 2026-04-08 making this rule explicit.
**Notes:** John takes notes in per-topic .md files under each section's notes/ folder. Python writing fluency is the main skill to rebuild — comprehension is ahead of production. Prefers lean/practical coverage of system design topics; pushes back when something feels theoretical and asks for justification — this is healthy. Honor that pattern: research, give honest answers, adjust scope rather than insisting. **Teaching pace (saved as feedback memory 2026-04-08):** never dump all stages of a sub-topic at once; teach one chunk at a time with confirmation between chunks. **Recurring weak spot — arithmetic execution:** confirmed across two sessions; reasoning is consistently ahead of math under pressure. Reinforce on every numeric question: write each step, give the actual number, sanity-check magnitude. **"Knows the rule, hasn't internalized implications" (new 2026-04-08):** when John recites a principle, push past the rule statement and force prediction of concrete failure modes — that's the mid→senior gap. Strong architectural instincts deserve explicit naming when they appear so the vocabulary sticks. Two-pointer (opposite-direction) is now unblocked in JS; same-direction still bookmarked for Phase 2.
