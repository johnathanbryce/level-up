...
- **Hash-set-with-complement pattern (2026-04-21, REINFORCED 2026-04-22):** Pair Products (Two Sum variant with multiplication) required heavy scaffolding on 2026-04-21. Pair with Difference K on 2026-04-22 — ALSO required heavy scaffolding. John defaulted back to "iterate and compare" nested-loop framing, struggled to articulate "compute specific complement, lookup in set." Hit the `if x or y in set` Python gotcha. Got the check-before-add timing wrong (same-index collision). Needed the solution written for him + line-by-line walkthrough before the pattern clicked — but trace afterwards was clean. **Third rep across 3 sessions has not locked this.** Priority: every Python warmup for the next several sessions should reinforce hash-map/set patterns. John has explicitly requested more reps on this. Do NOT declare the pattern learned.
