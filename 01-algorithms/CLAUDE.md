...
| Frequency counting (count-then-inspect) | 2026-04-08 (Valid Anagram, both langs) | 2026-05-09 (headliner_votes.py) | **MOSTLY LOCKED** | Formal lesson delivered: Counter, .items(), two-pass structure, failure modes. First formal drill B+ — one spec-reading miss (sort missed), one hint (.items() vs .get()). Next encounter: drill rep without hints → if clean, LOCKED. |
