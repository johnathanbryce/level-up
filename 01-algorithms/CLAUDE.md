...
- **Hash-set-with-complement pattern (2026-04-21):** Pair Products (Two Sum variant with multiplication) required heavy scaffolding. John did not recognize the structural match with Two Sum despite having solved Two Sum before. Pattern NOT locked — needs one more rep on a similar problem before marking complete. Watch for the same struggle if a similar complement-lookup problem appears.
