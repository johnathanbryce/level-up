...
- **Hash-set-with-complement pattern (2026-04-21, REINFORCED 2026-04-22 — 4 FAILED REPS):** Four consecutive failures now across Two Sum (JS, 2026-04-08), Pair Products (Python, 2026-04-21), Pair with Difference K (Python, 2026-04-22). John consistently defaults to "iterate and compare" nested-loop framing, struggles to articulate "compute specific complement, lookup in set." Same Python bugs recur: `if x or y in set` gotcha (parses as `if x or (y in set)`), check-before-add timing error causing same-index collision. In two of the four attempts, needed the solution written for him before the pattern clicked. Trace afterwards always clean. **Pattern is NOT recognizable to him as a structural family.** Priority: every Python warmup for the foreseeable should drill this pattern with variation (sum, product, difference, complement-in-range). John has explicitly requested more reps. Do NOT declare the pattern learned without at least 2 consecutive solves WITHOUT solution being written for him.
